...
import { apiAddTask } from '../../services/tasks';
...
export default function TaskScreen({navigation}) {
    ...
    const submit = async (data,actions) => {
        try{
            handlingLoading();
            const result = await apiAddTask(data);
            console.log(result);
            if(result.code === 200){
                Alert.alert(
                    "Succsess",
            ...
            <KeyboardAwareScrollView  showsVerticalScrollIndicator={false} bounces={false} >
                <View>
                    <View className="mb-5 space-y-1">
                        <Text className="text-3xl font-bold">Add Task</Text>
                    ...
                    <View className="mb-6">
                        <Input
                            ...
                            value={recent.length === 0 ? '' : recent[0].emoji}
